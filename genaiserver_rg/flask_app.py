...
def get_response():
    ...
    try:
        ...
        g.db.execute('INSERT INTO chat_messages (chat_id, sender, message, timestamp) VALUES (?, ?, ?, ?)', (chat_id, 'The Learing Chat', chat_text, datetime.now()))
...
def create_chat():
    ...
    username = session['username']
    g.db = connect_db()
    cur = g.db.execute('SELECT userid FROM users WHERE username = ?', (usernamefrom flask import Flask, render_template, redirect, url_for, request, session, flash, g, jsonify
import sqlite3
from datetime import datetime
import os
from dotenv import load_dotenv
...
def create_chat():
    ...
    create_new_chat(user_id, model_id, data['title'], model_name)
    g.db.close()
    return jsonify({"message": "Chatcreated successfully"}), 201
